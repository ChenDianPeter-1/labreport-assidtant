...
8. **Reference gate**: 完成初步检索和 `reference/reference-notes.md` 后必须停止，提示用户自行继续搜索、补齐、删除或确认 `reference/` 中的资料。未收到用户明确确认前，不得继续生成预习报告。
9. 用户确认 `reference/` 后，先定位报告模板文件。优先读取 `source/` 中的模板；若模板仍在实验文件夹根目录，也可读取根目录模板。读取模板中与预习报告对应的章节要求，至少包括实验目的、实验原理、实验装置/设备/试剂、实验步骤或预习准备工作。
10. 在写报告前建立模板上下文：逐条摘录模板对应位置的要求，并为每条要求匹配讲义中的来源段落、可用 `reference/` 资料和需要避免的越权内容。没有模板依据的内容不得写入预习报告主体。
    - 对“实验原理”章节，模板上下文必须先判断模板是否要求“简明扼要”。若是，则只保留核心反应、适用近似、关键测量关系和后续数据处理公式，不把讲义中的背景展开、仪器原理细节或冗长推导整段搬入报告主体。
11. 按模板逐项生成本地 HTML 文件，默认路径为当前实验文件夹的 `reports/prelab.html`。HTML 中每个主体章节必须对应模板中的一个要求，不得只按模型自定结构泛写。
12. 输出时只报告本地 HTML 文件路径，不要把它称为“网站”。
13. 暂停给用户确认，必要时按模板或教师要求修改。

各章节写作原则：

- **实验目的**：来自讲义，不编造讲义外目的。
- **实验原理**：优先简明扼要，保留核心公式和关键逻辑链；公式以讲义为准。若模板要求简明说明，则压缩为“反应是什么、为何可用该动力学模型、测量量如何对应浓度、怎样由公式求目标量”四个必要点，删除不影响后续数据处理的背景铺陈。
...
- 必须在用户确认 `reference/` 后才能生成 HTML。
- 生成 HTML 前必须先读取模板对应章节，把模板要求写入当前写作上下文。
- HTML 应严格对应模板中的预习报告要求，通常包括实验目的、实验原理、实验装置/设备/试剂、实验步骤或预习准备工作。
- 如果模板章节名称和讲义章节名称不一致，以模板章节为输出结构，以讲义章节为内容来源。
