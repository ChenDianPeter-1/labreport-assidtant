...
- 初步检索和 `reference/reference-notes.md` 完成后必须停止，等待用户自行补齐或确认 `reference/`，不得直接继续生成预习报告。
- 用户确认 `reference/` 后，必须先读取报告模板中与预习报告对应的位置，把模板要求逐项写入上下文，再按模板逐项生成 HTML。
- 预习 HTML 的章节结构以模板为准；讲义和 `reference/` 只能作为内容来源和补充依据。
