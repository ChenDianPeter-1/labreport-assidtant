...
9. 用户确认 `reference/` 后，提取讲义中的目的要求、仪器与试剂、实验原理、实验步骤。
10. 生成一个本地 HTML 文件，默认路径为当前实验文件夹的 `reports/prelab.html`。
11. 输出时只报告本地 HTML 文件路径，不要把它称为“网站”。
12. 暂停给用户确认，必要时按模板或教师要求修改。
